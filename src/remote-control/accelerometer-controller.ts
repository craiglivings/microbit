import { basic, Button, input, led, maqueen, pins, radio } from './mockRobot';
...
const calcAccSpeed = (value: number) => {
    let speed = value + 180;
    speed = speed < 90 ? 90 : speed;
    speed = speed > 270 ? 270 : speed;
    return speed;
};
...
const speedController = new AnalogStick(90, 270);
const directionController = new AnalogStick(90, 270);
...
basic.forever(() => {
    if (isRunning) {
        ...
        if (isHost) {
            let pitch = calcAccSpeed(-input.rotation(Rotation.Pitch));
            let roll = calcAccSpeed(input.rotation(Rotation.Roll));
            speedController.setValue(pitch);
            directionController.setValue(roll);
